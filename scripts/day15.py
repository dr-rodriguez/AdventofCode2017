...
(generator A uses 16807; generator B uses 48271), and then keep the remainder of dividing that
resulting product by 2147483647. That final remainder is the value it produces next.
...
def get_next(start, factor, divisor):
...
for i in range(int(40e6)):
    startA = get_next(startA, factorA, divisor)
    startB = get_next(startB, factorB, divisor)
    binA = '{:016b}'.format(startA)
    binB = '{:016b}'.format(startB)
    # print(startA, startB)
    # print(binA, binB)
    if binA[-16:] == binB[-16:]:
        count += 1
...
to consider 5 million pairs. (Using the values from the example above, after five million pairs,
the judge would eventually find a total of 309 pairs that match in their lowest 16 bits.)

After 5 million pairs, but using this new generator logic, what is the judge's final count?
"""

def get_next2(start, factor, divisor, check):
    value = (start * factor) % divisor
    if value % check == 0:
        return value, True
    else:
...
for i in range(int(5e6)):
    ...
    while not passA:
        startA, passA = get_next2(startA, factorA, divisor, 4)
    while not passB:
        startB, passB = get_next2(startB, factorB, divisor, 8)
    binA = '{:016b}'.format(startA)
    binB = '{:016b}'.format(startB)
    # print(startA, startB)
    # print(binA, binB)
