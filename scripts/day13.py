...
and then the scanners move one step. If there is a scanner at the top of the layer as your packet enters it,
you are caught. (If a scanner moves into the top of its layer while you are there,
...
its scanner was at the top when you entered it. You are not caught in layer 1,
...
with open('data/day13.txt', 'r') as f:
    for line in f:
        elem = line.split(':')
        my_input[int(elem[0])] = int(elem[1])
...
for i in range(0, 99):
    ...
    if r == 1:
        print('Caught at step {}'.format(i))
        severity += my_input.get(i, 0) * i
...
# My attempt failed since it took too long to continuously loop over each step
# region BAD
def reset_firewall():
    global firewall
    global my_input
...
step = 0
while step < 99:
    v = firewall.get(step, [[0]])
    r = v[0][0]

    if r == 1:
        print('Resetting ', delay, step)
        reset_firewall()
        step = 0
        delay += 1
        temp = delay
...
# Alternate attempt using reddit help
# The key thing is to realize that the firewall is at position 0 every 2*depth-2
caught = False
delay = 0
while True:
    caught = False
    for i in my_input.keys():
        if (i+delay) % (2*my_input[i] - 2) == 0:
            caught = True
            break
    if not caught:
        print(delay)
