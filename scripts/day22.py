...
grid = {}
for j, line in enumerate(my_input.split('\n')):
    for i, v in enumerate([s for s in line]):
        grid[(i, j)] = 1 if v == '#' else 0

# At center and up = 0
virus = (len(line)//2, len(line)//2, 0)
...
for j, line in enumerate(my_input.split('\n')):
    for i, v in enumerate([s for s in line]):
        grid[(i, j)] = 3 if v == '#' else 0

# 0: clean, 1: weakened, 2: flagged, 3: infected
...
for i in range(10000000):
    ...
    if node == 3:  # infected: right
        d = (d + 1) % 4
    elif node == 0:  # clean: left
        d = (d - 1) % 4
    elif node == 2:  # flagged: reverse
        d = (d + 2) % 4

    if node == 3:  # infected -> flagged
        grid[(x, y)] = 2
    elif node == 2:  # flagged -> clean
        grid[(x, y)] = 0
    elif node == 0:  # clean -> weakened
        grid[(x, y)] = 1
    elif node == 1:  # weakened -> infected
        grid[(x, y)] = 3
        count += 1
