...
because it's full of garbage. You look down at the stream; rather than water,
...
# region BAD
# THIS PRODUCES WRONG ANSWER
def pre_process(my_input):
    # Replace ! accordingly
    ind = my_input.find('!')
    while ind != -1:
        my_input = my_input[:ind] + '__' + my_input[ind+2:]
        ind = my_input.find('!')

    return my_input

def drop_garbage(my_input):
    pattern = r'\<(.*?)\>'
    garbage = re.findall(pattern, my_input)
    while len(garbage) > 0:
        # for g in garbage:
        #     print(g)
        #     my_input = my_input.replace(g, '')
        # print(len(garbage), garbage)
        my_input,n = re.subn(pattern, '', my_input)
        garbage = re.findall(pattern, my_input)


    return my_input

def find_group(my_input, lvl=1):
    groups = re.findall(r'\{(.*)\}', my_input)
    if len(groups) == 0:
        return 0
    for g in groups:
        temp = 0
        if g.startswith('{{'):
            return lvl + find_group(g, lvl+1)
        else:
            for s in g.split(','):
                temp += find_group(s, lvl+1)
        return lvl + temp

print(my_input)
my_input = pre_process(my_input)
print(my_input)
my_input = drop_garbage(my_input)
print(my_input)
print(find_group(my_input))
# THIS ANSWER IS INCORRECT!
# endregion

# Re-trying based on hints from reddit
# Go character by character this time
